...
from messages import first_message, chanel_check_text
from keyboards import lessons_menu, less_btn, check_channel
...
def start(update, context):
    ...
    user = check_status(user_id)
    update.message.reply_text(text=first_message)
    ...
    if user is None:
        ...
        if member.status in ["member", "administrator", "creator"]:
            ...
            update.message.reply_text(
                text=chanel_check_text,
                reply_markup=InlineKeyboardMarkup(check_channel(CHANNEL))
            )
